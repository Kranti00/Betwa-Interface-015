document.addEventListener("DOMContentLoaded", function() {
    const cardsContainer = document.querySelector('.unique-cards');
    let cardIndex = 0;
    ...
    function navigateCarousel(direction) {
        const cardWidth = document.querySelector('.unique-card').offsetWidth;
        const totalCards = document.querySelectorAll('.unique-card').length;
    ...
    document.querySelector('.unique-prev').addEventListener('click', () => navigateCarousel(-1));
    document.querySelector('.unique-next').addEventListener('click', () => navigateCarousel(1));
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const exploreCategoriesContainer = document.querySelector('.unique-explore-categories');
    ...
    rows.forEach(row => {
        ...
        rowDiv.classList.add('unique-category-row');
        
        row.forEach(categoryName => {
            const category = categoriesData.find(cat => cat.name === categoryName);
            if (category) {
                const categoryDiv = document.createElement('div');
                categoryDiv.classList.add('unique-category');
